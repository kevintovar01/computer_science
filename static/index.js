...
class Indice_Primario {
	...
	insertar(clave, nombre = "", apellido = "") {
		if (this.reg_insertados < this.cant_registros) {
			...
			this.estructura[this.reg_insertados].update_data(
				clave,
				nombre,
				apellido
			);
			...
			return `El elemento se ha insertado correctamente en la posición ${(pos % this.get_registros_por_bloque()) + 1
				} del índice ${Math.floor(pos / this.get_registros_por_bloque()) + 1
				}`;
		} else {
	...
	buscar(clave) {
		if (!this.isEmpty()) {
			...
			return `El elemento con la clave ${clave}, de nombre ${this.estructura[pos].nombre
				} ${this.estructura[pos].apellido
				}, se ha encontrado en la posición ${(pos % this.get_registros_por_bloque()) + 1
				} del índice ${Math.floor(pos / this.get_registros_por_bloque()) + 1
				}`;
		} else {
			return "El elemento no se encuentra en la estructura";
	...
	showStructure() {
		...
		for (let i = 0; i < this.estructura.length; i++) {
			...
			let item = this.estructura[i];
			row.innerHTML = `
                <td style="width: 13px; backgroundColor: #ffc07d">${i + 1}</td>
                <td>${item.clave || ""}</td>
                <td>${item.nombre || ""}</td>
                <td>${item.apellido || ""}</td>
            `;
			datosTable.appendChild(row);
			if ((i + 1) % this.get_registros_por_bloque() == 0) {
                ...
                <td style="border: 0px;"></td>
            `;
				datosTable.appendChild(emptyRow);
			}
		}
		...
		for (let i = 0; i < this.est_indices.length; i++) {
			const row = document.createElement("tr");
			let item = this.est_indices[i];
			row.innerHTML = `
                <td style="width: 13px; backgroundColor: #ffc07d">${i + 1}</td>
                <td>${item.clave || ""}</td>
			...
			if ((i + 1) % this.get_reg_indice_por_bloque() == 0) {
				const emptyRow = document.createElement("tr");
				emptyRow.innerHTML = `
                <td style="border: 0px;"></td>
                <td style="border: 0px;"></td>
                <td style="border: 0px;"></td>
            `;
				indicesTable.appendChild(emptyRow);
			}
		}
...
class Indice_Secundario {
	...
	get_bloques_indice_necesarios() {
		return Math.ceil(
			this.cant_registros / this.get_reg_indice_por_bloque()
		);
	}
	...
	insertar(clave, nombre = "", apellido = "") {
		if (this.estructura[this.cant_registros - 1].clave === null) {
			...
			this.actualizar_indices();
			let pos = this.buscar_pos(clave);
			return `El elemento se ha insertado correctamente en la posición 1 del índice ${pos + 1
				}`;
		} else {
			return "No se ha podido insertar el elemento debido a que la estructura está llena";
		}
	...
	buscar(clave) {
		if (this.estructura[0] !== null) {
			...
			return `El elemento con la clave ${clave}, de nombre ${this.estructura[pos].nombre
				...
				}, se ha encontrado en la posición 1 del índice ${pos + 1}`;
		} else {
			return "El elemento no se encuentra en la estructura";
		}
	}
...
class Indice_Primario_Multinivel {
	constructor(cap_bloque, cant_registros, long_registro, long_indice) {
		...
		this.estructura = new Array(
			Math.ceil(
				Math.log(this.get_bloques_necesarios()) /
				Math.log(this.get_reg_indice_por_bloque())
			) + 1
		).fill(null);
	...
	actualizar_indices() {
		if (this.cant_registros !== 1) {
			...
			for (
				...
				this.estructura[1][aux].update_data(
					this.estructura[0][i].clave,
					i + 1
				);
				aux += 1;
			}
	...
	isEmpty() {
		return this.estructura[0][0].clave == null;
	}
	...
	insertar(clave, nombre = "", apellido = "") {
		if (this.estructura[0][this.cant_registros - 1].clave === null) {
			...
			let pos = this.buscar_pos(clave);
			let aux = `El elemento con la clave ${clave} se ha insertado correctamente en la posición ${(pos % this.get_registros_por_bloque()) + 1
				} del nivel 0 de datos`;

			if (this.cant_registros !== 1) {
				aux += `, en la posición ${(Math.floor(pos / this.get_registros_por_bloque()) %
				...
				if (this.estructura.length > 1) {
					...
					for (let i = 2; i < this.estructura.length; i++) {
						aux += `, en la posición ${Math.floor(pos / this.get_reg_indice_por_bloque()) +
							1
							} del nivel ${this.estructura.length - i} de índices`;
						pos =
							Math.floor(pos / this.get_reg_indice_por_bloque()) +
							1;
					}
				}
			}
	...
	eliminar(clave) {
		if (!this.isEmpty()) {
			...
			alert(this.estructura[0][0].clave); return "El elemento no se encuentra en la estructura";
		}
	}
	...
	buscar(clave) {
		if (!this.isEmpty()) {
			...
			let aux = `El elemento con la clave ${clave}, de nombre ${this.estructura[0][pos].nombre
				} ${this.estructura[0][pos].apellido
				} se ha encontrado satisfactoriamente en la posición ${(pos % this.get_registros_por_bloque()) + 1
				} del nivel 0 de datos`;
	...
	showStructure() {
		...
		for (let i = 0; i < this.estructura[0].length; i++) {
			const row = document.createElement("tr");
			let item = this.estructura[0][i];
			row.innerHTML = `
                    <td style="width: 13px; backgroundColor: #ffc07d">${i + 1
				}</td>
                    <td>${item.clave || ""}</td>
                    <td>${item.nombre || ""}</td>
                    <td>${item.apellido || ""}</td>
		...
		if (this.cant_registros !== 1) {
			...
			for (let i = 0; i < this.estructura[1].length; i++) {
				const row = document.createElement("tr");
				let item = this.estructura[1][i];
				row.innerHTML = `
                    <td style="width: 13px; backgroundColor: #ffc07d">${i + 1
					}</td>
                    <td>${item.clave || ""}</td>
			...
			if (this.get_bloques_indice_necesarios() > 1) {
              ...
              `;
					...
					for (let j = 0; j < this.estructura[i].length; j++) {
						const row = document.createElement("tr");
						let item = this.estructura[i][j];
						row.innerHTML = `
                          <td style="width: 13px; backgroundColor: #ffc07d">${j + 1
							}</td>
                          <td>${item.clave || ""}</td>
                          <td>${item.direccion || ""}</td>
                      `;
						...
						if (
							(j + 1) % this.get_reg_indice_por_bloque() ==
							0
						) {
							const emptyRow = document.createElement("tr");
							emptyRow.innerHTML = `
...
class Indice_Secundario_Multinivel {
		...
		if (cant_registros !== 1) {
			...
			if (this.get_bloques_indice_necesarios() > 1) {
				...
				for (let i = 2; i < this.estructura.length; i++) {
					this.estructura[i] = new Array(
						Math.ceil(
							aux / this.get_reg_indice_por_bloque()
						)
					)
						.fill(null)
						.map(() => new Indice());
	...
	get_bloques_necesarios() {
		return Math.ceil(
			this.cant_registros / this.get_registros_por_bloque()
		);
	}
	...
	insertar(clave, nombre = "", apellido = "") {
		if (
			this.estructura[0][this.cant_registros - 1].clave ===
			null
		) {
			for (let i = 0; i < this.reg_insertados; i++) {
				if (this.estructura[0][i].clave === clave) {
			...
			let pos = this.buscar_pos(clave);
			let aux = `El elemento con la clave ${clave} se ha insertado correctamente en la posición 1 del nivel 0 de datos`;

			if (this.cant_registros !== 1) {
				aux += `,en la posición ${pos %
					this.get_reg_indice_por_bloque() +
					1
					} del nivel ${this.estructura.length - 1
					} de índices`;

				if (this.estructura.length > 1) {
					pos = Math.floor(pos / this.get_reg_indice_por_bloque());
					...
					for (
						...
						aux += `, en la posición ${pos % this.get_reg_indice_por_bloque() + 1} del nivel ${this.estructura.length - i} de índices`;
						pos = Math.floor(pos / this.get_reg_indice_por_bloque());
					}
				}
			}
	...
	buscar(clave) {
		if (!this.isEmpty()) {
			...
			let aux = `El elemento ${clave}, de nombre ${this.estructura[0][pos].nombre
				} ${this.estructura[0][pos].apellido
				} se ha encontrado satisfactoriamente en la posición 1 del nivel 0 de datos`;
...
function create_structure() {
	...
	cant_registros = parseInt(
		document.getElementById("form-structure_cantRegistros")
			.value
	);
	...
	if (
		...
		switch (indexType) {
			case 1:
				estructura = new Indice_Primario(
					cap_bloque,
					cant_registros,
					long_registro,
					long_indice
				);
				document.getElementById("form-key").style.display =
					"flex";
				document.getElementById("structure").style.display =
					"flex";
				estructura.showStructure();
				break;
			case 2:
				estructura = new Indice_Secundario(
					cap_bloque,
					cant_registros,
					long_registro,
					long_indice
				...
				break;
			case 3:
				estructura = new Indice_Primario_Multinivel(
					cap_bloque,
					cant_registros,
					long_registro,
					long_indice
				...
				break;
			case 4:
				estructura = new Indice_Secundario_Multinivel(
					cap_bloque,
					cant_registros,
					long_registro,
					long_indice
...
function startIndexPrimaryOneLevel() {
	...
	document.getElementById("form-structure").style.display =
		"flex";
...
document
	.getElementById("indexPrimary_oneLevel")
	.addEventListener("click", startIndexPrimaryOneLevel);
document
	.getElementById("indexSecondary_oneLevel")
	.addEventListener("click", startIndexSecondaryOneLevel);
document
	.getElementById("indexPrimary_multipleLevel")
	.addEventListener("click", startIndexPrimaryMultipleLevels);
document
	.getElementById("indexSecondary_multipleLevel")
	.addEventListener("click", startIndexSecondaryMultipleLevels);